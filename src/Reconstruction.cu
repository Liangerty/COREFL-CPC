...
__device__ void MUSCL_reconstruct(const real *pv, real *pv_l, real *pv_r, int idx_shared, int n_var, int limiter) {
...
__device__ void NND2_reconstruct(const real *pv, real *pv_l, real *pv_r, int idx_shared, int n_var, int limiter) {
  for (int l = 0; l < n_var; ++l) {
    // \Delta_i = u_i - u_{i-1}; \Delta_{i+1} = u_{i+1} - u_i
    const real delta_i{pv[idx_shared * n_var + l] - pv[(idx_shared - 1) * n_var + l]};
    const real delta_i1{pv[(idx_shared + 1) * n_var + l] - pv[idx_shared * n_var + l]};
