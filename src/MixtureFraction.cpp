...
BilgerH::BilgerH(Inflow &fuel, Inflow &oxidizer, const Species &spec, int myid) :
  ...
  for (auto &[name, i]: spec.spec_list) {
    if (xk_fuel[i] > 1e-10) {
      snprintf(buffer, sizeof(buffer), "%.4f %s + ", xk_fuel[i], name.c_str());
      spec_eqn += buffer;
    }
  }
  snprintf(buffer, sizeof(buffer), "%.4f ( O2 ", o2_needed);
  spec_eqn += buffer;
  for (auto &[name, i]: spec.spec_list) {
    if (name == "O2") {
      continue;
    ...
    if (xk_oxidizer[i] > 1e-10) {
      snprintf(buffer, sizeof(buffer), "+ %.4f %s ", xk_o2_unity_oxi[i], name.c_str());
      spec_eqn += buffer;
    }
  }
  spec_eqn += ") = Products\n";
  std::string elem_eqn;
  snprintf(buffer, sizeof(buffer), "%.4f H + %.4f O -> P\n", nu_h, nu_o);
  elem_eqn += buffer;

  nuh_mwh = nu_h * 1.008;
  half_nuo_mwo = nu_o * 15.999 * 0.5;
  std::string coupling_func{"\\beta = "};
  snprintf(buffer, sizeof(buffer), "ZH/%.4f - ZO/%.4f\n", nuh_mwh, half_nuo_mwo);
  coupling_func += buffer;
  if (myid == 0) {
    snprintf(buffer, sizeof(buffer), "%s%s%s", spec_eqn.c_str(), elem_eqn.c_str(), coupling_func.c_str());
    printf("%s%s%s", spec_eqn.c_str(), elem_eqn.c_str(), coupling_func.c_str());
...
BilgerCH::BilgerCH(Inflow &fuel, Inflow &oxidizer, const Species &spec, int myid) :
  ...
  spec_eqn += ") = Products\n";
  std::string elem_eqn;
  snprintf(buffer, sizeof(buffer), "%.4f C + %.4f H + %.4f O -> P\n", nu_c, nu_h, nu_o);
  elem_eqn += buffer;
  ...
  half_nuo_mwo = nu_o * 15.999 * 0.5;
  std::string coupling_func{"\\beta = "};
  snprintf(buffer, sizeof(buffer), "ZC/%.4f + ZH/%.4f - ZO/%.4f\n", nuc_mwc, nuh_mwh, half_nuo_mwo);
  coupling_func += buffer;
  if (myid == 0) {
    printf("%s%s%s", spec_eqn.c_str(), elem_eqn.c_str(), coupling_func.c_str());
    std::ofstream out("output/message/mixture_fraction_info.txt", std::ios::trunc);
...
void acquire_mixture_fraction_expression(const Species &spec, const real *fuel, const real *oxidizer,
  Parameter &parameter) {
  ...
  std::string elem_eqn;
  if (has_c) {
    snprintf(buffer, sizeof(buffer), "%.4f C + ", nu_c);
    elem_eqn += buffer;
  }
  snprintf(buffer, sizeof(buffer), "%.4f H + %.4f O -> P\n", nu_h, nu_o);
  elem_eqn += buffer;
  ...
  std::string coupling_func{"\\beta = "};
  snprintf(buffer, sizeof(buffer), "ZC/%.4f + ZH/%.4f - ZO/%.4f\n", nuc_mwc_inv, nuh_mwh_inv, half_nuo_mwo_inv);
  coupling_func += buffer;
  if (!has_c)
    nuc_mwc_inv = 0;
  else
