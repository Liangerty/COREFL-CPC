...
void cfd::InnerFace::register_boundary(const int dim, const int ngg) {
  for (int i = 0; i < dim; ++i) {
...
cfd::Block::Block(int _mx, int _my, int _mz, int _ngg, int _id, const Parameter &parameter) :
  ...
  metric{mx, my, mz, 9, _ngg}, bType_il(_my, _mz, 0), bType_ir(_my, _mz, 0), bType_jl(_mx, _mz, 0),
  bType_jr(_mx, _mz, 0), bType_kl(_mx, _my, 0), bType_kr(_mx, _my, 0) {
  ...
  for (int i = 1; i < mx - 1; ++i) {
    for (int j = 1; j < my - 1; ++j) {
      ...
      for (int k = k_min; k < k_max; ++k) {
        ...
        if (jac <= 0) {
          printf("Negative Jacobian from process %d\nBlock %d, index is (%d, %d, %d).\nStop simulation.\n", myid,
                 block_id, i, j, k);
          printf("dxd1=(%e - %e)/2=%e, dyd1=(%e - %e)/2=%e, dzd1=(%e - %e)/2=%e\n",
                 x(i + 1, j, k), x(i - 1, j, k), dxd1,
                 y(i + 1, j, k), y(i - 1, j, k), dyd1,
                 z(i + 1, j, k), z(i - 1, j, k), dzd1);
          printf("dxd2=(%e - %e)/2=%e, dyd2=(%e - %e)/2=%e, dzd2=(%e - %e)/2=%e\n",
                 x(i, j + 1, k), x(i, j - 1, k), dxd2,
                 y(i, j + 1, k), y(i, j - 1, k), dyd2,
                 z(i, j + 1, k), z(i, j - 1, k), dzd2);
          printf("dxd3=(%e - %e)/2=%e, dyd3=(%e - %e)/2=%e, dzd3=(%e - %e)/2=%e\n",
...
cfd::Mesh::Mesh(Parameter &parameter) :
  ...
  if (myid == 0)
    gxl::print_centered("Grid Information", 80, '*');
  ...
  read_inner_interface(myid);

  if (parallel) {
    read_parallel_interface(myid);
  }
...
void cfd::Mesh::read_grid(const int myid, const Parameter &parameter) {
  if (parameter.get_int("mesh_file_type") == 1) {
    ...
    if (parameter.get_int("gridIsBinary")) {
      ...
      MPI_File grd;
      char name[100];
      snprintf(name, sizeof(name), "./input/grid/grid%4d.dat", myid);
      MPI_File_open(MPI_COMM_SELF, name, MPI_MODE_RDONLY, MPI_INFO_NULL, &grd);
      ...
      // ASCII grid file
      char name[100];
      snprintf(name, sizeof(name), "./input/grid/grid%4d.grd", myid);
      std::ifstream grd(name, std::ios::in);
      std::string input;
...
void cfd::Mesh::read_boundary(int myid, bool need_extend_boundary, int periodic_label) {
  char name[100];
  snprintf(name, sizeof(name), "./input/boundary_condition/boundary%4d.txt", myid);
  std::ifstream grd(name, std::ios::in);
  std::string input{};
  for (int blk = 0; blk < n_block; ++blk) {
    std::getline(grd, input);
    ...
    for (int ii = 0; ii < bsz; ++ii) {
      ...
      // This part is used for discretizations near the boundaries. The periodic bc is regarded as an inner point, while others are seen as a physical boundary.
      // The inner points are treated with data transfer between blocks, while the physical boundaries are treated with boundary discretization schemes.
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  if (myid == 0) {
    printf("\tFinish reading boundary conditions.\n");
  }
}

void cfd::Mesh::read_inner_interface(const int myid) {
  char name[100];
  snprintf(name, sizeof(name), "./input/boundary_condition/inner%4d.txt", myid);
  std::ifstream grd(name, std::ios::in);
  std::string input{};
  ...
  for (int blk = 0; blk < n_block; ++blk) {
    ...
    for (int l = 0; l < n_face[blk]; ++l) {
      ...
      block[blk].inner_face[l].register_boundary(dimension, ngg);
    }
  }
  MPI_Barrier(MPI_COMM_WORLD);
  if (myid == 0) {
    printf("\tFinish reading inner communication faces.\n");
  }
  grd.close();
  delete[]n_face;
}
...
void cfd::Mesh::read_parallel_interface(const int myid/*, int ngg*/) {
  char name[100];
  snprintf(name, sizeof(name), "./input/boundary_condition/parallel%4d.txt", myid);
  std::ifstream grd(name, std::ios::in);
  std::string input{};
  ...
  if (blk_num1 != n_block) {
    printf("Read parallel error! In process %d, ", myid);
    printf("n_block in grid file is %d, in parallel file is %d\n", n_block, blk_num1);
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  delete[]n_face;
  MPI_Barrier(MPI_COMM_WORLD);
  if (myid == 0) printf("\tFinish reading parallel communication faces.\n");
  grd.close();
}
...
void cfd::Mesh::init_ghost_grid(const int myid, const bool parallel) {
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  if (myid == 0) {
    printf("\tFinish creating ghost grids.\n");
  }
}
