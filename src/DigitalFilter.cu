...
__global__ void compute_convolution_kernel(const real *y_scaled, ggxl::VectorField2D<real> *df_by,
  ggxl::VectorField2D<real> *df_bz, real dz_scaled, int iFace, int my, int ngg) {
  const int j = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x) - ngg;
  if (j >= my + ngg) {
    return;
  }
...
__global__ void perform_convolution_z(ggxl::VectorField2D<real> *df_fy, ggxl::VectorField2D<real> *df_bz,
