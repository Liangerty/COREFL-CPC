...
std::istream &gxl::getline_to_stream(std::ifstream &file, std::string &input, std::istringstream &line, Case u_l) {
  std::getline(file, input);
  trim_left(input);
  if (u_l == Case::upper) {
    for (auto &ch: input) {
...
void gxl::read_until(std::ifstream &file, std::string &input, std::string &&to_find, Case u_l) {
...
void gxl::print_centered(const char *text, int width, char fill_char) {
  const int text_length = static_cast<int>(std::strlen(text));
  if (text_length >= width) {
    printf("%s\n", text);
    return;
  }
  const int total_padding = width - text_length;
