...
template<MixtureModel mixture_model> void DPLUR(const Block &block, const DParameter *param, DZone *d_ptr, DZone *h_ptr,
  const Parameter &parameter, DBoundCond &bound_cond, real diag_factor) {
  ...
  compute_DQ_0<<<bpg, tpb>>>(d_ptr, param, diag_factor);
...
__global__ void compute_DQ_0(DZone *zone, const DParameter *param, real diag_factor) {
...
template void DPLUR<MixtureModel::Air>(const Block &block, const DParameter *param, DZone *d_ptr, DZone *h_ptr,
  const Parameter &parameter, DBoundCond &bound_cond, real diag_factor);

template void DPLUR<MixtureModel::Mixture>(const Block &block, const DParameter *param, DZone *d_ptr, DZone *h_ptr,
  const Parameter &parameter, DBoundCond &bound_cond, real diag_factor);
}
